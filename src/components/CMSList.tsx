...
const CMSList = ({
  ...
  useEffect(() => {
    const query = featuredOnly
      ? `*[_type == $collection && featured == true] | order(orderRank asc, _createdAt desc)`
      : `*[_type == $collection] | order(orderRank asc, _createdAt desc)`;
  ...
  return (
    <section id={collection} className="py-20 flex flex-col gap-10 mx-auto w-full">
      ...
      <div
        className={`grid grid-cols-1 sm:grid-cols-2 ${cols === 3 ? "md:grid-cols-3" : "md:grid-cols-2"
          } gap-x-4 gap-y-4`}
      >
